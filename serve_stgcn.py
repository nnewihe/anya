...
archive/train_serve_gcn.py and used at inference in archive/serve_detector.py.

Same role as serve_lstm.ServeLSTMDetector (drop-in interface: update(crop) /
reset() / score()), but models joint-to-joint anatomical structure via graph
convolution at each timestep before convolving over time, rather than
treating all landmarks as one flat per-frame feature vector.
"""
...
DEFAULT_MODEL_PATH      = os.path.join(_ARCHIVE_DIR, "serve_stgcn.pt")
...
class ServeSTGCNDetector:
    ...
    (0.0 before that). Same interface as serve_lstm.ServeLSTMDetector.
    """
