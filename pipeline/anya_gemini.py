...
class PointStartSystem:
    ...
    def process_frame(self, current_data: TrackData) -> MatchState:
        ...
        if self.state == MatchState.INACTIVE:
            ...
            #    px, py, pw, ph = current_data.far_player_bbox
            #    self.current_toss_roi = self._get_far_toss_roi(px, py, pw, ph)
            #    if self._detect_far_toss():
            #        self.state = MatchState.FAR_SERVE_IN_FLIGHT
            #        self.toss_detected_frame_idx = current_data.frame_idx
            #        print(f"[State] Far toss detected at frame {current_data.frame_idx}")
        ...
        #        self.state = MatchState.INACTIVE
        #        print(f"[State] Aborted Far Toss/Timeout: Reset to INACTIVE")
            ...
            if self.active_frame_counter >= (self.fps * 3):
                self.state = MatchState.INACTIVE
                self.active_frame_counter = 0
                print(f"[State] Resetting to INACTIVE")
...
def run_point_detector(video_path: str, output_path: str, ball_model_path: str, stride: int = 10):
    ...
    while cap.isOpened():
        ...
        # --- B. Gated Ball Inference (OPTIMIZED) ---
        ball_pos = None
        if system.current_toss_roi is not None:
            ...
            if c_right > c_left and c_bottom > c_top:
                ...
                # CRITICAL FIX: Lock imgsz to 256 for small crops to stop YOLO from upscaling
