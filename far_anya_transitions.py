...
class FarTransitionEngine:
    def __init__(self, fps: float, net_y_px: float):
        ...
        # ── WAITING → ARMED ───────────────────────────────────────────────
        # Condition 1 — far half: box centre cy < net_y_px − NET_BUFFER_PX
        self.NET_BUFFER_PX = 0.0    # no buffer — check directly against net line
        ...
        # Grace: tolerate short detection gaps without resetting the buffer
    ...
    def _check_waiting(self, history: deque) -> str:
        ...
        # ── Condition 1: far player is in the far half ────────────────────
        in_far_half = cy < self.net_y_px - self.NET_BUFFER_PX
        ...
        if self._waiting_debug_counter % 30 == 0:
            print(f"[FAR WAIT DBG] net_y={self.net_y_px:.0f}  "
                  f"cx={cx:.0f}  cy={cy:.0f}  "
                  f"ceil={self.net_y_px - self.NET_BUFFER_PX:.0f}  "
                  f"in_far_half={in_far_half}  "
        ...
        print(f"[FAR TRANSITION] WAITING -> ARMED. "
              f"Far player stationary for {span:.1f}s  "
              f"vel={velocity:.1f}px/s  cy={cy:.0f}  net_y={self.net_y_px:.0f}")
        self._vel_buffer.clear()
        self._vel_miss_frames = 0
        return "ARMED"
    ...
    def _check_armed(self, history: deque) -> str:
        ...
        # Update rolling movement history for fall-back check
        ...
        # Net displacement > threshold → player walked away → WAITING
